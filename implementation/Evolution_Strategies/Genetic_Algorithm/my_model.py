...
def get_model_specs(generation):
    ...
    if generation == 0:
      ...
      top_spec_1 = int(data['spec'][0])
      top_spec_2 = int(data['spec'][1])

      #print(top_spec)

      with open(f'./specs/{top_spec_1}.spec', 'rb') as f:
        rand_spec_1 = pickle.load(f)

      with open(f'./specs/{top_spec_2}.spec', 'rb') as f:
        rand_spec_2 = pickle.load(f)

      co_1 = random.randint(0, 3)
      co_2 = random.randint(4, 6)

      org_mat = []
      for i, (m1, m2) in enumerate(zip(rand_spec_1.original_matrix, rand_spec_2.original_matrix)):
        if i in [1, 2, 3]:
          m2[co_1:co_2] = m1[co_1:co_2]
        org_mat.append(m2.tolist())

      org_mat, org_ops = np.asarray(org_mat), rand_spec_1.original_ops
      print(co_1, co_2, '\n', rand_spec_1.original_matrix, '\n', rand_spec_2.original_matrix, '\n', org_mat)
    
      rand_spec = mutate_spec1(api.ModelSpec(org_mat, org_ops))
...
def get_model(**kwargs):
    ...
    specs = get_model_specs(generation=int(kwargs['generation']))
